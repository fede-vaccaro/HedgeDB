...
#include "db/merge/sst_stream_reader.h"
...
namespace hedge::db
{
        ...
        std::shared_ptr<db::sharded_page_cache> cache)
    {
        ...
        const size_t write_buffer_size = hedge::ceil_page_align(256 * KiB);
        ...
        sst_stream_reader_set rbufs(num_bufs);

        for(const auto* index : indices)
        {
            rbufs.emplace_back(*index,
                               tmc::current_executor(),
                               fs::stream_reader_config{
        ...
        {
            merge_entry_t entry{};
            sst_stream_reader* source{nullptr};
        };
        ...
        for(auto* rbuf = rbufs_begin; rbuf < rbufs_end; ++rbuf)
        {
            ...
            slots.emplace_back(std::move(new_keypair), rbuf);
        ...
        while(!key_heap.empty())
        {
            ...
            if(dedup.ready()) // For getting ready, we need to have pushed at least two entries
            {
                ...
                if(qf.has_value())
                    qf->insert(xxh64::hash(reinterpret_cast<const char*>(new_item.key.data()), new_item.key.size(), sst::QF_SEED));
        ...
        [[maybe_unused]] auto check_every_buf_is_eof = [](sst_stream_reader* rbufs_begin, sst_stream_reader* rbufs_end)
        {
            for(auto* it = rbufs_begin; it < rbufs_end; ++it)
            {
                if(!it->is_eof())
        ...
        [[maybe_unused]] auto check_key_count_matches = [&write_buffer, filtered_keys, &dedup](sst_stream_reader* rbufs_begin, sst_stream_reader* rbufs_end)
        {
            size_t total_keys = 0;
            for(auto* it = rbufs_begin; it < rbufs_end; ++it)
