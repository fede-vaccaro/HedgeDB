...
namespace hedge::db
{
    ...
    struct latency_collector
    {
        ...
        void init(size_t n_threads, size_t ops_per_thread)
        {
    ...
    {
        std::vector<std::pair<std::string, std::shared_ptr<latency_collector>>> collectors;

        void add(std::string label, const std::shared_ptr<latency_collector>& collector)
        {
            collectors.emplace_back(std::move(label), collector);
        }
