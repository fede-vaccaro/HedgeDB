...
class uring_reader
{
    ...
    void run()
    {
        ...
        while(true)
        {
            ...
            while (!this->_pending_requests_buffer.empty()) 
            {
                auto& [req_id, req] = this->_pending_requests_buffer.front();
                ...
                io_uring_prep_read(sqe, req.fd, this->_mem_aligned_buffers[ref_buffer], PAGE_SIZE, req.offset);

                sqe->user_data = ref_buffer;

                in_flight_requests.emplace_back(req_id, std::move(req));

                this->_pending_requests_buffer.pop_front();
            }

            if (auto ready = io_uring_sq_ready(&this->_ring); ready > 0) 
            {
            ...
            auto ret = io_uring_wait_cqe_nr(&this->_ring, &cqe, in_flight_requests.size());
            ...
            io_uring_for_each_cqe(&this->_ring, head, cqe) 
            {
                ...
                auto* src = static_cast<uint8_t*>(this->_mem_aligned_buffers[cqe->user_data]);
                auto* src_end = src + PAGE_SIZE;
                std::copy(src, src_end, buffer.begin());
                
                req.second.callback(std::move(buffer)); // todo defer to executor
...
public:
    uring_reader()
    {
        int ret = io_uring_queue_init(QUEUE_DEPTH, &this->_ring, IORING_SETUP_SQPOLL);
        ...
        this->_worker = std::thread(&uring_reader::run, this);
    }
...
int main() {
    ...
    for(size_t i = 0; i < NUM_RANDOM_READS; ++i) {
        unsigned long long random_page_index = distribution(generator);
        ...
        auto req = request{
            ...
            .size = PAGE_SIZE,
            .callback = [&wg, i](std::vector<uint8_t>&& buffer) {
