...
int main(int argc, char* argv[])
{
    ...
    const auto FLUSH_INDICES_LIMIT = 1;
    const auto INDEX_MAX_SIZE = N_UUIDS;
    ...
    for (size_t i = 0; i < N_UUIDS; ++i)
    {
        ...
        if (mem_indices.size() >= FLUSH_INDICES_LIMIT)
        {
            ...
            {
                std::cerr << "Failed to flush indices: " << partitioned_indices.error().to_string() << '\n';
                return 1;
            }
    ...
    {
        auto prefix = hedgehog::db::extract_prefix(uuid);
