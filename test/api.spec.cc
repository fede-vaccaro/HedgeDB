...
namespace hedge::db
{
    struct api_test : public ::testing::TestWithParam<std::tuple<size_t, size_t, size_t>>
    {
        void SetUp() override
        {
    ...
    TEST_P(api_test, api_comprehensive_test)
    {
        ...
        auto maybe_db = api::create_database(this->_base_path, config);
        ...
        for(size_t i = 0; i < this->N_KEYS; ++i)
        {
            ...
            auto value = api_test::make_random_vec_seeded(this->PAYLOAD_SIZE, i);
        ...
        for(size_t i = 0; i < this->N_KEYS; ++i)
        {
            db->get(this->_uuids[i],
                    [i, &read_wg, this, &number_of_errors](expected<std::vector<uint8_t>> maybe_value)
                    {
                        if(!maybe_value)
                        {
                            number_of_errors++;
                            std::cerr << "An error occurred during retrieval for key " << this->_uuids[i] << ": " << maybe_value.error().to_string() << std::endl;
                            read_wg.decr();
                            return;
                        }
                        ...
                        auto expected_value = api_test::make_random_vec_seeded(this->PAYLOAD_SIZE, i);
    ...
    INSTANTIATE_TEST_SUITE_P(
        ...
        [](const testing::TestParamInfo<api_test::ParamType>& info)
        {
